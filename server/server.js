  ...
  .then(() => {
    ...
    server.use(async (ctx, next) => {
      ...
      const redirectTo =
        ...
        `/install/auth?shop=${encodeURIComponent(shop)}&host=${encodeURIComponent(host)}`;

      console.log("[TOPLEVEL] /auth/toplevel hit", { shop, hasHost: !!host, redirectTo });

      const apiKey = SHOPIFY_API_KEY || process.env.SHOPIFY_API_KEY || process.env.NEXT_PUBLIC_SHOPIFY_API_KEY;
