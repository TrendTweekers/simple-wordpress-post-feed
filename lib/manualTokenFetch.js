...
export const waitForShopify = async (maxWaitMs = 5000) => {
  ...
  // ✅ CRITICAL: Check if shopify.config exists (App Bridge initialization check)
  // If window.shopify exists but config is missing, App Bridge didn't initialize properly
  if (window.shopify) {
    if (!window.shopify.config) {
      console.error('[MANUAL TOKEN] ❌ CRITICAL: window.shopify exists but shopify.config is missing!');
      console.error('[MANUAL TOKEN] This means App Bridge did not initialize properly');
      console.error('[MANUAL TOKEN] Host parameter:', hostParam);
      return false;
    }
    
    // If idToken function exists, we're good
    if (typeof window.shopify.idToken === 'function') {
      console.log('[MANUAL TOKEN] ✅ App Bridge initialized correctly (config exists, idToken available)');
      return true;
    }
  }
  ...
  return new Promise((resolve) => {
    const checkInterval = setInterval(() => {
      if (window.shopify && window.shopify.config && typeof window.shopify.idToken === 'function') {
        ...
        if (window.shopify && !window.shopify.config) {
          console.error('[MANUAL TOKEN] ❌ Timeout: window.shopify exists but config is missing');
          console.error('[MANUAL TOKEN] Host parameter:', hostParam);
        } else {
          console.warn('[MANUAL TOKEN] ⚠️ Timeout waiting for window.shopify.idToken()');
        }
        resolve(false);
      }
