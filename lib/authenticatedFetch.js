...
const triggerReauth = (reauthUrl, appBridgeApp = null) => {
  ...
  try {
    ...
    // Final fallback: force top-level redirect for embedded iframes
    console.warn(`[AUTH] App Bridge not available, using top-level redirect`);
    ...
    // Force top-level redirect on error
    if (typeof window !== "undefined" && window.top) {
      window.top.location.href = fullUrl;
    } else {
      window.location.href = fullUrl;
